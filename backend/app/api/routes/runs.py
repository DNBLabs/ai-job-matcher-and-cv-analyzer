...
from app.domain.run_outcomes import classify_run_failure, failure_message_for
...
def _run_to_response(analysis_run: AnalysisRun) -> RunResponse:
    ...
    return RunResponse(
        ...
        failure_reason=(
            r.value
            if (r := classify_run_failure(analysis_run.status, analysis_run.source_failures_json))
            else None
        ),
