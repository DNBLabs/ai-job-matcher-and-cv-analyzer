...
# GPT-4o list pricing (USD per 1M tokens) for per-listing Analysis Run scoring.
# Source: https://developers.openai.com/api/docs/pricing (verified 2026-06-09).
_GPT4O_INPUT_USD_PER_MILLION = 2.50
_GPT4O_OUTPUT_USD_PER_MILLION = 10.00
...
def estimate_gpt4o_usd(*, prompt_tokens: int, completion_tokens: int) -> float:
    """Estimate USD cost for a GPT-4o call from token usage counts.
    ...
    Returns:
        float: Rounded estimated USD cost for FinOps audit metadata.
    """
    input_cost = (prompt_tokens / 1_000_000) * _GPT4O_INPUT_USD_PER_MILLION
    output_cost = (completion_tokens / 1_000_000) * _GPT4O_OUTPUT_USD_PER_MILLION
    return round(input_cost + output_cost, 6)
