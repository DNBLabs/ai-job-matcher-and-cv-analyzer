...
- Blob Storage: service endpoint + **deny-by-default network ACL** (ACA subnet + transient operator/CI IP), MI-only data-plane access, no shared keys — **not** a private endpoint (ADR-0007, matches the Key Vault posture; a private endpoint cannot be Terraform-provisioned from outside the VNet)
...
- **2026-06-11:** Transactional email transport — **Microsoft 365 shared mailbox via Graph API** (`sendMail`), authenticated by Container App **Managed Identity** (no stored API key), replacing Resend. `Mail.Send` application permission **must** be scoped to the single shared mailbox via an Exchange Online **Application Access Policy**. From = `noreply@dnblabs.co.uk`. See "Run Notification" §Email delivery and IMPLEMENTATION_PLAN Open Questions / Task 27.
- **2026-06-13:** Transactional email implemented (Task 27) — `GraphApiNotificationPort` (M365 shared mailbox `sendMail`, MI-authenticated) replaces the Resend adapter, which is removed. `Mail.Send` is granted to **both** the API and Worker MIs and **must** be constrained to `noreply@dnblabs.co.uk` by an Exchange **Application Access Policy** (out-of-band; no Terraform resource). See `docs/adr/0005-graph-sendmail-managed-identity-email.md` and `docs/ops/RUNBOOK.md`.
- **2026-06-16:** CV blob storage networking — **service endpoint + deny-by-default network ACL** (ACA subnet + transient operator/CI IP), **not** a private endpoint, overriding §Network "public access disabled". A private-endpoint-only account cannot be `terraform apply`'d from outside the VNet (the provider polls the blob data plane on create), which broke both local applies and the Task 29 GitHub-hosted pipeline. Matches the existing Key Vault posture; shared keys stay disabled (MI/AAD only). Also added `storage_use_azuread = true` to both stacks' azurerm provider (required once shared keys are off). See `docs/adr/0007-blob-storage-service-endpoint-not-private-endpoint.md`.
- **2026-06-15:** Zero-touch deploy scope (Task 29) — automate the **application release** path only; one-time IAM stays out of the per-merge pipeline so it never holds tenant-admin Graph permissions. `deploy.yml` runs on CI-green `main` (OIDC only; SHA-tagged images; `terraform apply` + `az containerapp update` revision pin). The deploy OIDC identity is IaC in `infra/bootstrap`; the Graph `Mail.Send` grant moves to a one-time operator-applied `infra/grants` stack; the **Exchange Application Access Policy stays a documented manual admin step** because `Connect-ExchangeOnline` app-only needs a long-lived certificate, which would violate the OIDC-only guardrail (line 98). See `docs/adr/0006-zero-touch-deploy-oidc.md`.
- **2026-06-13:** Service Bus networking — **Basic SKU with public endpoint**, overriding §Network "Service Bus: private endpoint". Private Link requires Premium (~£500+/mo), breaking the £75 cap. Compensating controls: app send/receive via Managed Identity + queue-scoped RBAC (Data Sender / Data Receiver); a single **Listen-only** queue-scoped SAS rule feeds the KEDA scaler only (ACA `custom_scale_rule` supports connection-string auth only). See `docs/adr/0004-service-bus-basic-no-private-endpoint.md`.
...
**Email delivery (prod requirement):** Transactional email (magic link + run-complete) is sent from a **Microsoft 365 shared mailbox via the Graph API** (`POST /users/{mailbox}/sendMail`). `EMAIL_FROM` is the shared mailbox address **`noreply@dnblabs.co.uk`**; SPF/DKIM/DMARC are already established on the `dnblabs.co.uk` M365 domain, so deliverability does not depend on warming a new sender domain. The Container App authenticates to Graph with its **Managed Identity** — no API key is stored. **Least privilege:** the app's `Mail.Send` *application* permission grants send-as-any-mailbox by default and **must** be constrained to only the `noreply@dnblabs.co.uk` shared mailbox with an Exchange Online **Application Access Policy** (`New-ApplicationAccessPolicy`). Verify at build time that app-only `sendMail` to the shared mailbox does not require a per-mailbox licence. Plan a post-deploy real-inbox deliverability check. Local dev uses the `log` adapter (no actual send); sign-in is completed with a minted verify link.
